...
public interface FriendshipsRepository extends CrudRepository<Friendship, Long> {
    ...
    @Query("SELECT f.receiver.email FROM Friendship f WHERE f.sender.email LIKE ?1 AND f.accepted = false")
    List<String> getSentRequests(String email);

    @Query("SELECT f.sender.email FROM Friendship f WHERE f.receiver.email LIKE ?1 AND f.accepted = false")
